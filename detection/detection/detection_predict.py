...
def preprocess(path_to_img, img_size, device):
    ...
    if orig_img.shape[0] > orig_img.shape[1]:
        orig_img = cv2.transpose(orig_img)
        orig_img = cv2.flip(orig_img, flipCode=1)
...
def get_img_predict(model, path_to_img, device, img_size):
    ...
    return img_predict,orig_shape,tensor_shape



def preprocess_flip(path_to_img, img_size, device):
    orig_img = cv2.imread(path_to_img)
    if orig_img.shape[0] > orig_img.shape[1]:
        orig_img = cv2.transpose(orig_img)
        orig_img = cv2.flip(orig_img, flipCode=1)
...
def postprocess_flip(
    ...
    for pred in out:    
        ...
        box[:,2] /= orig_shape[1]
        box[:,3] /= orig_shape[0]
        x1 = box[:,0].detach().clone()
        x2 = box[:,2].detach().clone()
        box[:,0] = 1 - x2
        box[:,2] = 1 - x1
...
def get_img_predict_flip(model, path_to_img, device, img_size):
    img_tensor, orig_shape, tensor_shape = preprocess_flip(path_to_img, img_size, device)
    with torch.no_grad():
        out, _ = model(img_tensor, augment=False)
    img_predict = postprocess_flip(out, tensor_shape, orig_shape, path_to_img)
    return img_predict,orig_shape,tensor_shape
...
def get_detection_solution(
    ...
    detect_model = attempt_load(path_to_weights[:2], map_location=device)
    detect_model2 = attempt_load(path_to_weights[2:4], map_location=device)
    detect_model3 = attempt_load(path_to_weights[4:], map_location=device)
#     detect_model2.eval()
#     detect_model3.eval()
#     detect_model.eval()
    result = []
    for file_name in tqdm(os.listdir(path_to_test_images)):
        img_predict,orig_shape,tensor_shape = get_img_predict(
            detect_model, os.path.join(path_to_test_images, file_name), device, img_size
        )
        img_predict2,orig_shape,tensor_shape = get_img_predict(
            detect_model2, os.path.join(path_to_test_images, file_name), device, 2176
        )
        img_predict3,orig_shape,tensor_shape = get_img_predict_flip(
            detect_model, os.path.join(path_to_test_images, file_name), device, img_size
        )
        img_predict4,orig_shape,tensor_shape = get_img_predict_flip(
            detect_model2, os.path.join(path_to_test_images, file_name), device, 2176
        )
        img_predict5,orig_shape,tensor_shape = get_img_predict(
            detect_model3, os.path.join(path_to_test_images, file_name), device, 2688
        )
        img_predict6,orig_shape,tensor_shape = get_img_predict(
            detect_model3, os.path.join(path_to_test_images, file_name), device, 2560
        )
        ...
        for i in range(len(boxes)):
            #coord = scale_coords(tensor_shape[2:], boxes[i], orig_shape).round()
            box = boxes[i]
            coord = [(box[0] * orig_shape[1]),(box[1] * orig_shape[0]),(box[2] * orig_shape[1]),(box[3] * orig_shape[0])]
            w = coord[2] - coord[0]
            h = coord[3] - coord[1]
            x = coord[0] 
            y = coord[1]
